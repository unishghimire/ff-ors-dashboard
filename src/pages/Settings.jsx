...
import { getConnectionToken, setConnectionToken, setGatewayUrl, gatewayUrl, resetToDefaults } from '../config'
import { Settings as SettingsIcon, Save, Key, Globe, Check, Copy, Link, Code, Shield, Zap, Database } from 'lucide-react'
...
export default function Settings() {
  ...
  function saveUrl() { setGatewayUrl(customUrl); setSavedUrl(true); setTimeout(() => setSavedUrl(false), 3000) }
  function resetDefaults() { resetToDefaults(); setToken(getConnectionToken()); setCustomUrl(''); setSavedToken(true); setTimeout(() => setSavedToken(false), 2000) }
  ...
  const fullGatewayUrl = gwUrl.startsWith('http') ? gwUrl : `${window.location.origin}${gwUrl}`

  const exampleCode = `// JavaScript — call the ORS Gateway
fetch('${fullGatewayUrl}', {
  method: 'POST',
  headers: {
    ...
    <div className="p-6 space-y-6 page-enter">
      ...
      <div className="card p-5 space-y-4" style={{ borderColor: 'var(--ors-accent)', borderWidth: 2 }}>
        ...
        <p className="text-sm" style={{ color: 'var(--ors-text-muted)' }}>
          Your ORS API key. Copy and paste it into any external software that needs to connect to the ORS API.
        </p>
        <div>
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg" style={{ background: 'var(--ors-bg)' }}>
          ...
          <p className="text-xs" style={{ color: 'var(--ors-text-muted)' }}>
            Keep this token secure. Include it as the <code style={{ fontFamily: 'monospace' }}>X-API-Key</code> header in every request to the gateway.
          </p>
        </div>
      </div>
      ...
      <div className="card p-5 space-y-4">
        ...
        <p className="text-sm" style={{ color: 'var(--ors-text-muted)' }}>
          The backend runs as a Vercel serverless function using Firebase Firestore. All operations go through this single endpoint.
        </p>
        <div>
          <label className="text-xs font-medium block mb-1.5" style={{ color: 'var(--ors-text-muted)' }}>GATEWAY URL</label>
          <div className="flex gap-2">
            <div className="input flex items-center justify-between flex-1" style={{ fontFamily: 'monospace', overflowX: 'auto' }}>
              <span className="text-sm">{fullGatewayUrl}</span>
            </div>
            <button onClick={() => copyToClipboard(fullGatewayUrl, setCopiedGateway)} className="btn-secondary flex items-center gap-2 whitespace-nowrap">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="px-3 py-2 rounded-lg" style={{ background: 'var(--ors-bg)' }}>
            <div className="text-xs" style={{ color: 'var(--ors-text-muted)' }}>DATABASE</div>
      ...
      <div className="card p-5 space-y-4">
        ...
        <div className="space-y-2">
          {[
            ...
            { op: 'seed_data', desc: 'Initialize default tournament data (call once)' },
            ...
            { op: 'capture_and_process', desc: 'Capture frame → OCR → push (match_id, image_data)' },
      ...
      {/* Advanced: Custom Gateway URL */}
      <div className="card p-5 space-y-4">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <Globe className="w-5 h-5" style={{ color: 'var(--ors-text-muted)' }} /> Custom Gateway URL (Advanced)
        </h2>
        <p className="text-sm" style={{ color: 'var(--ors-text-muted)' }}>
          Default is <code style={{ fontFamily: 'monospace' }}>/api/gateway</code> (same Vercel deployment). Override only if you host the backend separately.
        </p>
        <div className="flex gap-2">
          <input className="input" placeholder="/api/gateway" value={customUrl} onChange={e => setCustomUrl(e.target.value)} />
          <button onClick={saveUrl} className="btn-primary flex items-center gap-2 whitespace-nowrap">
            <Save className="w-4 h-4" /> Save
          </button>
          {savedUrl && <span className="text-sm flex items-center gap-1" style={{ color: 'var(--ors-green)' }}><Check className="w-4 h-4" /> Saved!</span>}
        </div>
        <button onClick={resetDefaults} className="text-xs underline" style={{ color: 'var(--ors-text-muted)' }}>
          Reset to defaults
        </button>
      </div>
      ...
      <div className="card p-5 space-y-4">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <Key className="w-5 h-5" style={{ color: 'var(--ors-text-muted)' }} /> Update Token (Advanced)
        </h2>
        <p className="text-sm" style={{ color: 'var(--ors-text-muted)' }}>
          If you rotate the ORS_API_KEY on the server, update it here to match.
        </p>
        <div className="flex gap-2">
          <input className="input" placeholder="ORS-..." value={token} onChange={e => setToken(e.target.value)} style={{ fontFamily: 'monospace' }} />
          <button onClick={saveToken} className="btn-primary flex items-center gap-2 whitespace-nowrap">
            <Save className="w-4 h-4" /> Save
          </button>
